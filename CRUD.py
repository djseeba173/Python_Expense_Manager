...
def add():
    ...
    while(rta == "y"):
        ...
        mycursor.execute(
            "INSERT INTO Operation (amount, category, description) VALUES (%s, %s, %s)", (money, category, description))
        db.commit()
...
def view():
    order = input("Do you want to order by (amount or category)? ")
    if order == "amount":
        moneyOrder = input("Do you want to order by (asc or desc)? ")
        if moneyOrder == "asc":
            mycursor.execute("SELECT * FROM Operation ORDER BY amount ASC")
        else:
            mycursor.execute("SELECT * FROM Operation ORDER BY amount DESC")
    elif order == "category":
        categoryOrder = input("What category do you want to order by? ")
        mycursor.execute("SELECT * FROM Operation WHERE category = %s", (categoryOrder,))
...
def edit():
    ...
    mycursor.execute(
        "SELECT * FROM Operation WHERE description = %s", (searched,))
    ...
    if row is not None:
        ...
        mycursor.execute("UPDATE Operation SET description = %s, category = %s, amount = %s WHERE description = %s",
                         (newDescription, newCategory, newAmount, searched))
        db.commit()
    else:
...
def delete():
    ...
    if row is not None:
        ...
        if rta == "y":
            mycursor.execute(
                "DELETE FROM Operation WHERE description = %s", (searched,))
            db.commit()
...
if __name__ == "__main__":
    ...
    decision = input(
        "What do you want to do? (a)dd, (v)iew, (e)dit, (d)elete, (q)uit: ")
    while(decision != "q"):
        clear()
        if(decision == "a"):
            add()
